...
export function MiniMap({ imageUrl, zoom, position, containerSize, onNavigate }: MiniMapProps) {
  ...
  return (
    <div className="absolute bottom-4 right-4 z-20 rounded-lg border-2 border-primary/50 bg-background/95 p-2 backdrop-blur-sm">
      <canvas ref={canvasRef} width={200} height={150} className="cursor-pointer rounded" onClick={handleClick} />
      <p className="mt-2 text-center text-xs text-muted-foreground">Mini-Map Navigator</p>
    </div>
  )
}
